...
class Annotator:
    def __init__(self, data_path="/workspaces/Prius2audioscene/dataset",lidar_sensor="LIDAR_TOP", pcd_prefix="rosbag__", version="v1.0-test", threshold=0.4):
        """
        ...
        self.scene_name = 'output' #'rosbag2_2025_08_21-13_56_45'
        self.specific_bag_name = "rosbag2_2025_08_21-14_13_15_10km"
        self.data_path = os.path.join(data_path, self.scene_name)
        ...
        self.insntance_path = os.path.join(self.data_path, self.dataset_version, "instance.json")
        ...
        self.class_mapping = {
            ...
            17:"vehicle.car",
            23:"vehicle.truck",
            24:"flat.drivable_surface",
            28:"static.manmade",
        } # TODO: Know what happens with the classes in MS3D
    ...
    def read_multiple_annotations(self):
        ...
        """
        # samples_path = os.path.join(self.data_path,"labels")
        if not os.path.exists(self.labels_path):
            print(f"Folder not found: {self.labels_path}")
            return
        annotations = {}     
        for filename in os.listdir(self.labels_path):
            samples_filepath = os.path.join(self.labels_path, filename)
            samples = self.read_samples(samples_filepath)
            if samples is None:
                continue
            for sample in samples:
                print(f"Reading sample with name: {sample.get('name')}")
                print(sample.get('name') ,self.scene_name)
                if sample.get('name') == self.specific_bag_name:
                    print(f"Updating sample {sample.get('name')}")
                    annotations.update(sample)
        return annotations

    def read_samples(self,samples_filepath):
        # Check if the PCD file exists
        """
        Reads the samples from the exported JSON file.

        Args:
            samples_filepath (str): Path to the samples JSON file.

        Returns:
            dict: A dictionary containing the samples.
        """
        if not os.path.exists(samples_filepath):
            print(f"Folder not found: {samples_filepath}")
            return     
        with open(samples_filepath, 'r') as f:
            result = json.load(f)
        print(f"Read annotation from {samples_filepath}")
        dataset = result.get("dataset")
        if dataset:
            print(f"Found 'dataset' with name: {dataset.get('name')}")
            samples = dataset.get("samples")
            return samples
        else:
            print(f"No 'dataset' key found in the JSON file: {samples_filepath}")
        return {}
    ...
    def process_annotations(self,annotation_dictionary):
        ...
        instance_to_json = []
        result_to_json = []
        label_dictionary = annotation_dictionary.get("labels", {}).get("ground-truth", {})
        print(f"Processing {len(label_dictionary)} labels")
        attributes_dictionary = label_dictionary.get("attributes", {})
        frames_dictionary = attributes_dictionary.get("frames", {})
        print(f"Processing {len(frames_dictionary)} frames")

        for frame in tqdm.tqdm(frames_dictionary):
            timestamp = frame.get("timestamp")
            annotations = frame.get("annotations")
            print(f"Processing annotation for timestamp: {timestamp}")
        ...
        write_json(self.sample_annotation_path,annotation_to_json)
        write_json(self.insntance_path,instance_to_json)
    ...
    def create_final_results_format(self, results_list):
        ...
        sample_annotation_sample_tokens = [item["sample_token"] for item in read_json(self.sample_annotation_path)]
    ...
    def process_sample_annotation(self, timestamp, annotations):
        ...
        timestamp_str = str(int(timestamp * 1e6))
        pcd_filename = f"{self.pcd_prefix}{timestamp_str}.pcd.bin"
        # print(f"Looking for PCD file: {pcd_filename}")
        ...
        print(f"Annotations have length: {len(annotations)}")
    ...
    def extract_box_information(self,annotation, ego_pose):
        ...
        """
        print(f"Extracting box information from annotation: {annotation}")
        label = np.float64(annotation.get("category_id", 0)) # Default to ignore class if label not found
        print(f"Label: {label}")
        #Score is not provided in the segments annotations, so we set it to 1.0
        score = 1.0 # np.float64(annotation[8])
        center_dict = annotation.get("position", [0,0,0])
        print(f"Center: {center_dict}")
        center = [np.float64(center_dict.get("x", 0)),np.float64(center_dict.get("y", 0)),np.float64(center_dict.get("z", 0))]
        print(f"Center after conversion: {center}")
        dimensions_dict = annotation.get("dimensions", [1,1,1])
        print(f"Dimensions: {dimensions_dict}")
        # lwh = np.float64(annotation[3:6] * 1)
        wlh = [np.float64(dimensions_dict.get("y", 0)), np.float64(dimensions_dict.get("x", 0)), np.float64(dimensions_dict.get("z", 0))]
        print(f"Dimensions after conversion (wlh): {wlh}")
        # add a small value to avoid division by zero and assumes no pitch and roll
        axis_angles = np.array([0, 0, np.float64(annotation.get("yaw", 0)) + 1e-10])
        ## TODO: CHECK
        rot = Rotation.from_euler('xyz', axis_angles, degrees=False)

        # center2, rot = transform_to_global(ego_pose, center, rot)
        rot_list = rot.as_quat(scalar_first=True).tolist()
        return center, wlh, rot_list, label, score
